...
#from streamer import register_streamer_routes  # Import your new file

app = Flask(__name__)

CORS(app)

#register_streamer_routes(app)
...
def camera_and_record_worker():
    ...
    # CRITICAL: Forces OpenCV to only keep the newest frame. Prevents massive stream delay.
...
'''def start_cloudflare_background(port=5000):
    def run():
        ...
        if not path:
            ...
            return
        subprocess.Popen([path, "tunnel", "--url", f"http://localhost:{port}", "--no-autoupdate"],
...
def start_cloudflare_background(port=5000):
    def run():
        ...
        if not path:
            # Common default paths if shutil.which fails
        ...
        # Start the tunnel and capture stderr (where the URL is logged)
        process = subprocess.Popen(
            [path, "tunnel", "--url", f"http://localhost:{port}", "--no-autoupdate"],
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT, # Redirect stderr to stdout to catch the logs
            text=True
        )

        # Read the output line by line to find the public URL
        for line in process.stdout:
            if "trycloudflare.com" in line:
                # Use regex to extract the URL from the log line
                match = re.search(r"https?://[^\s']+", line)
                if match:
                    print("\n" + "="*50)
                    print(f"🌍 PUBLIC URL: {match.group(0)}")
                    print("="*50 + "\n")
                # We found the link, but we keep the loop going so the process doesn't die
            
            # Optional: Print logs for debugging if the tunnel fails
            # print(line.strip()) 

    # Run in background thread so it doesn't block the Flask server
    threading.Thread(target=run, daemon=True).start()
...
if __name__ == '__main__':
    ...
    # Run server (waitress/gevent recommended for prod, but Flask threaded is OK here)
